...
def _condition_summary(cond: Dict[str, Any]) -> Dict[str, Any]:
    ...
    in source, and the ASM/constant metadata when present. Guard ORDER is positional —
    a setter's conditions list is already ordered — so no "order" field (it defeated
    interning: the same guard at a different position was a distinct object). blockId
    is dropped — the lineage output strips codeBlocks, so there is nothing for it to
    reference."""
...
class _CondInterner:
    """Per-chain dedup table for guard objects. A real chain doc emitted 1.13M
    condition dicts of which only 170k were DISTINCT — the shared chain-hop and
    function-prefix guards get re-stamped into every setter they gate (the top
    guard repeated 2,271×). With the table, each setter carries ordered integer
    indexes into ``table`` and every distinct guard is serialized exactly once."""

    def __init__(self) -> None:
        self.table: List[Dict[str, Any]] = []
        self._ids: Dict[str, int] = {}

    def add(self, cond: Dict[str, Any]) -> int:
        key = json.dumps(cond, sort_keys=True, default=str)
        i = self._ids.get(key)
        if i is None:
            i = self._ids[key] = len(self.table)
            self.table.append(cond)
        return i


def _setter_summary(setter: Dict[str, Any],
                    interner: Optional[_CondInterner] = None) -> Dict[str, Any]:
    ...
    if conds:
        # The full ordered guard list (was a bare count, which forced a second
        # /vbt/trace call to see the expressions). With an interner, the setter
        # carries ordered integer indexes into the doc's top-level "conditions"
        # table instead of inline copies (schemaVersion 2); without one, the
        # guard objects ride inline as before.
        if interner is not None:
            out["conditions"] = [interner.add(_condition_summary(c)) for c in conds]
        else:
...
def _build_lineage(chain_out: Dict[str, Any], variable: str, *,
    ...
    # The root's setters are NOT embedded here — the chain doc already carries them
    # as its top-level "rootSetters" (they were serialized twice: 33MB each in a
    # real doc). The node keeps the count so graph consumers see the fan-in.
    root_node = {
        "name": variable, "role": "root", "terminal": False,
        "setterCount": len(root.get("setters") or []),
    ...
    for entry in dep_entries:
        ...
        if node is None:
            node = {"name": name, "role": "dependent",
                    ...
                    "setters": [_setter_summary(s, interner)
    ...
    for name, node in nodes.items():
        ...
        # classification rides on the node; the evidence strings live ONLY in the
        # "terminals" list (they were duplicated on both).
...
def run_lineage(job_id: str, options: Dict[str, Any],
    ...
    # pretty_json=True restores indented per-chain docs (debugging); compact is the
    # default — indent=2 alone doubled a real 542MB chain doc.
    pretty_json = bool(options.get("pretty_json", topts.get("pretty_json", False)))
    ...
    try:
        ...
        for language in languages:
            ...
            for i, chain in enumerate(chains):
                ...
                # One guard table per chain doc: setters reference guards by index
                # (schemaVersion 2) — every distinct guard serializes exactly once.
                interner = _CondInterner()
                lineage = _build_lineage(out, variable, io_stems=io_stems, const_names=const_names,
                                         input_prefixes=input_prefixes, ws_prefixes=ws_prefixes,
                                         missing_ctx=missing_ctx, interner=interner)
                ...
                root_setters = [_setter_summary(s, interner)
                ...
                if split_chains:
                    ...
                    doc = {"schemaVersion": 2, "variable": variable, "language": language,
                           "chain_index": i, "files": stems, "rootSetters": root_setters,
                           "lineage": lineage, "conditions": interner.table}
                    # compact by default — pretty-printing doubled a real 542MB doc
                    (out_dir / fname).write_text(
                        json.dumps(doc, indent=2) if pretty_json
                    ...
                    lang_trace["chains"].append({
                        "schemaVersion": 2, "chain_index": i, "files": stems,
                        "lineage": lineage, "rootSetters": root_setters,
                        "conditions": interner.table,
                    })
