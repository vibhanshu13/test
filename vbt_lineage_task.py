...
def _condition_summary(cond: Dict[str, Any]) -> Dict[str, Any]:
    """One guard, flattened for the lineage output: the ordered expression text, where it
    sits in source, and the ASM/constant metadata when present. blockId is dropped — the
    lineage output strips codeBlocks, so there is nothing for it to reference."""
    ...
    f = loc.get("file")
    out: Dict[str, Any] = {"order": cond.get("order"),
...
def _setter_summary(setter: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if conds:
        ...
        # /vbt/trace call to see the expressions). Split-chain mode bounds the
        # per-file size, so the guards ride along with the setter they gate.
            ...
            for i, chain in enumerate(chains):
                ...
                if split_chains:
                    ...
                    doc = {"variable": variable, "language": language, "chain_index": i,
                           "files": stems, "rootSetters": root_setters, "lineage": lineage}
                    ...
                    lang_trace["chains"].append({
                        "chain_index": i, "files": stems, "lineage": lineage,
                        "rootSetters": root_setters,
                    })
