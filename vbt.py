...
class VbtLineageRequest(BaseModel):
    ...
    pretty_json: bool = Field(
        default=False,
        description="Indent the per-chain JSON files (debugging). Default is compact "
                    "serialization — indentation alone doubled a real 542MB chain doc.")
    trace_options: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Tunables: max_dep_var_depth (default 10), max_chains_to_trace (default 50), "
                    ...
                    "(max_routes, max_paths, …), scratch_local_prefixes (default ['WRK']: ASM "
                    "scratch work fields whose setter search is module-local; [] disables), "
                    "plus classifier marker overrides (db_read_markers, user_input_markers, "
