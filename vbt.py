...
# worker's threadpool and returns the result inline. Skips the Celery dispatch
...
@router.post(
    "/vbt/trace-sync",
    summary="Run a VBT trace synchronously and return the result inline",
    description=(
        "Synchronous variant of POST /vbt/trace. Skips Celery and returns the full "
        "trace result JSON in the HTTP response. Identical input/output otherwise. "
        "Use this for low-latency clients; use the async endpoint for long-running "
...
def submit_vbt_trace_sync(
    ...
    # Load the emitted JSON so the client gets the full result in one request.
    output_files = manifest.get("outputs") or []
    result_blob: Optional[Dict[str, Any]] = None
    if output_files:
        first = Path(output_files[0])
        if first.is_file() and first.suffix == ".json":
            try:
