...
_SETTER_EMIT_CACHE_MAX = 100_000
...
def _cfg_for_stem(cfg_cache: Dict[str, List[Dict]], stem: str, asm_dir: Path) -> List[Dict]:
    """Return cached cfg_extract output for one C++ stem.

    Avoid ``dict.setdefault(stem, run_cfg_extract(...))`` because Python evaluates
    the default argument before calling setdefault, so it reruns cfg_extract even
    when the stem is already cached.
    """
    cfg = cfg_cache.get(stem)
    if cfg is None:
        cfg = run_cfg_extract(str(Path(asm_dir) / f"{stem}.cpp"))
        cfg_cache[stem] = cfg
    return cfg
...
def _function_return_setters(fn_name: str, midx: ModifierIndex, asm_dir: Path,
    ...
    for stem in defs:
        ...
        if not cpp_path.exists():
            continue
        fns = _cfg_for_stem(cfg_cache, stem, asm_dir)
...
def _param_binding_setters(dep: DepVarRef, disc_stem: str, asm_dir: Path,
    ...
    if not disc_cpp.exists():
        return [], []
    disc_cfg = _cfg_for_stem(cfg_cache, disc_stem, asm_dir)
    ...
    for stem in sorted(cand):
        ...
        if not cpp_path.exists():
            continue
        cfg2 = _cfg_for_stem(cfg_cache, stem, asm_dir)
...
def _filter_chain_scoped(setters, dep: DepVarRef, chain_files: Set[str],
    ...
    def _fn_at(stem: str, line: int) -> Optional[str]:
        if cfg_cache is None or asm_dir is None:
            return None
        cfg = _cfg_for_stem(cfg_cache, stem, Path(asm_dir))
...
def _clone_dep_refs(refs: List[DepVarRef]) -> List[DepVarRef]:
    """Return fresh dep refs from cached setter materialization.

    The BFS mutates some DepVarRef instances later (notably function-output
    def_file resolution), so cached children must never be reused by identity.
    """
    out: List[DepVarRef] = []
    for d in refs or []:
        out.append(replace(
            d,
            found_at=dict(d.found_at or {}),
            indirection=copy.deepcopy(d.indirection),
        ))
    return out


def _setter_emit_key(s: SetterSite, asm_max_levels: int) -> Tuple:
    """Exact cache key for one setter-card materialization.

    Keep every SetterSite field that can influence conditions/children/output in
    the key. The cache is trace-local via _NodeCtx, so source freshness is handled
    by the caller's per-trace context.
    """
    return (
        s.language,
        s.file_stem,
        ...
        s.value,
        s.setter_code_chunk,
        bool(s.constant_source),
        bool(s.prior_value_required),
        s.role,
        s.flipc_alias_of,
        s.hardware_source,
        s.function or "",
        bool(s.is_declaration),
        int(asm_max_levels),
    )
...
class _NodeCtx:
    ...
    setter_emit_cache: Dict[
        Tuple,
        Tuple[Dict[str, Any], List[DepVarRef], bool, Optional[Tuple[str, Dict]]],
    ] = field(default_factory=dict)
    setter_emit_stats: Dict[str, int] = field(default_factory=lambda: {"cards": 0, "hits": 0, "misses": 0})
    setter_search_cache: Dict[Tuple[str, str, str, str, str], List[SetterSite]] = field(default_factory=dict)
    setter_search_stats: Dict[str, int] = field(default_factory=lambda: {"queries": 0, "hits": 0, "misses": 0, "files": 0})
...
def _emit_setter_run(setter_jsons, seen, relevant, owner_name, depth, *,
    ...
    for s in relevant:
        ...
        if dkey in seen:        # same write reached via multiple paths / read-sites
            continue
        seen.add(dkey)
        ctx.setter_emit_stats["cards"] = ctx.setter_emit_stats.get("cards", 0) + 1
        ck = _setter_emit_key(s, ctx.asm_max_levels)
        cached = ctx.setter_emit_cache.get(ck)
        if cached is not None:
            ctx.setter_emit_stats["hits"] = ctx.setter_emit_stats.get("hits", 0) + 1
            template, cached_child, _asm_trunc, block_item = cached
            if store is not None and block_item is not None:
                bid, block = block_item
                if bid not in store.blocks:
                    store.blocks[bid] = copy.deepcopy(block)
            child = _clone_dep_refs(cached_child)
            setter_entry = copy.deepcopy(template)
            setter_entry["setterId"] = len(setter_jsons) + 1
        else:
            ctx.setter_emit_stats["misses"] = ctx.setter_emit_stats.get("misses", 0) + 1
            asm_trunc = False   # ASM depth-cap truncation flag (set only in the ASM branch)
            block_item: Optional[Tuple[str, Dict]] = None
            if s.language == "cpp":
                cpp_path = ctx.asm_dir / f"{s.file_stem}.cpp"
                fns = _cfg_for_stem(ctx.cfg_cache, s.file_stem, ctx.asm_dir)
            ...
            if store is not None and dblk and dblk in store.blocks:
                block_item = (dblk, copy.deepcopy(store.blocks[dblk]))
            ...
            template = copy.deepcopy(setter_entry)
            template["setterId"] = 0
            if len(ctx.setter_emit_cache) >= _SETTER_EMIT_CACHE_MAX:
                ctx.setter_emit_cache.pop(next(iter(ctx.setter_emit_cache)))
            ctx.setter_emit_cache[ck] = (
                template,
                _clone_dep_refs(child),
                asm_trunc,
                copy.deepcopy(block_item) if block_item is not None else None,
            )
...
def _target_setters_for_dep(tname: str, tlang: str, dep: DepVarRef,
                            ctx: _NodeCtx, disc_stem: str) -> List[SetterSite]:
    """Raw candidate setter search for one target, cached per trace context.

    This preserves the existing semantics: same candidate stems, same precise
    setter finder, same later chain-scope/off-chain filtering. It only avoids
    repeating that candidate-file lookup when many d3 nodes share the same tail
    or alias target in the same read-site shape.
    """
    fp_key = dep.qualified if (tlang == dep.language and tlang == "cpp" and dep.qualified) else ""
    disc_key = disc_stem if tlang == dep.language else ""
    key = (tname, tlang, dep.language, disc_key, fp_key)
    ctx.setter_search_stats["queries"] = ctx.setter_search_stats.get("queries", 0) + 1
    hit = ctx.setter_search_cache.get(key)
    if hit is not None:
        ctx.setter_search_stats["hits"] = ctx.setter_search_stats.get("hits", 0) + 1
        return list(hit)
    ctx.setter_search_stats["misses"] = ctx.setter_search_stats.get("misses", 0) + 1
    ...
    ctx.setter_search_stats["files"] = ctx.setter_search_stats.get("files", 0) + len(cand_sorted)

    out: List[SetterSite] = []
    if tlang == "cpp":
        tail = tname.split(".")[-1].split("->")[-1]
        fp = [dep.qualified] if fp_key else None
        for stem in cand_sorted:
            ...
            if not found:
                continue
            _cfg = _cfg_for_stem(ctx.cfg_cache, stem, ctx.asm_dir)
            for _s in found:
        ...
        for stem in cand_sorted:
            out += find_asm_setters_in_file(tname, stem, ctx.blueprint_dir, ctx.asm_dir)

    if len(ctx.setter_search_cache) >= 100_000:
        ctx.setter_search_cache.pop(next(iter(ctx.setter_search_cache)))
    ctx.setter_search_cache[key] = list(out)
    return out
...
def _compute_node(item: Tuple[str, DepVarRef, int]) -> _NodeOut:
    ...
    for tname, tlang in targets:
        setters += _target_setters_for_dep(tname, tlang, dep, ctx, disc_stem)
...
def trace_dependents(
    ...
    *,
    ...
    _timing = bool(_dbg) or _LOG.isEnabledFor(logging.INFO)
    if _LOG.isEnabledFor(logging.INFO):
        _LOG.info(
            "dependent expansion workers: %d%s",
            _workers,
            " (route-memo warmup enabled)" if _workers > 1 and route_engine is not None else "",
        )
    ...
    def _warm_route_memo(items: List[Tuple[str, DepVarRef, int]]) -> None:
        ...
        depth = items[0][2] if items else 0
        if _timing:
            _tc = time.perf_counter()
        per_chunk = _pool.for_each_chunk(_collect_routes_chunk, items)
        collect_s = time.perf_counter() - _tc if _timing else 0.0
        uniq: List[Tuple] = []
        ...
        if _timing:
            _LOG.info(
                "  dep warm [d%d]: collect n=%d chunks=%d uniq_q=%d %.2fs",
                depth, len(items), len(per_chunk), len(uniq), collect_s,
            )
        if not uniq:
            return
        ...
        if _timing:
            _LOG.info(
                "  dep warm [d%d]: resolve uniq_q=%d %.2fs",
                depth, len(uniq), time.perf_counter() - _tr,
            )
    ...
    def _compute_batch(items: List[Tuple[str, DepVarRef, int]]) -> List[_NodeOut]:
        ...
        if _timing:
            ...
            _emit_before = dict(ctx.setter_emit_stats)
            _search_before = dict(ctx.setter_search_stats)
        r = [_compute_node(it) for it in items]
        if _timing:
            _emit_after = ctx.setter_emit_stats
            _cards = _emit_after.get("cards", 0) - _emit_before.get("cards", 0)
            _hits = _emit_after.get("hits", 0) - _emit_before.get("hits", 0)
            _misses = _emit_after.get("misses", 0) - _emit_before.get("misses", 0)
            _search_after = ctx.setter_search_stats
            _sq = _search_after.get("queries", 0) - _search_before.get("queries", 0)
            _sh = _search_after.get("hits", 0) - _search_before.get("hits", 0)
            _sm = _search_after.get("misses", 0) - _search_before.get("misses", 0)
            _sf = _search_after.get("files", 0) - _search_before.get("files", 0)
            _LOG.info(
                "  dep compute [d%d]: n=%d serial %.2fs (batch total %.2fs; setter search q=%d hit=%d miss=%d files=%d; setter cards=%d cache hit=%d miss=%d)",
                depth, len(items), time.perf_counter() - _t_compute,
                time.perf_counter() - _t_batch, _sq, _sh, _sm, _sf, _cards, _hits, _misses,
            )
        return r
    ...
    try:
        while q:
            ...
            if _timing and batch:
                _LOG.info(
                    "  dep merge [d%d]: batch=%d computed=%d next_queue=%d %.2fs",
                    batch[0][2], len(batch), len(compute_items), len(q),
                    time.perf_counter() - _t_merge,
                )
    finally:
