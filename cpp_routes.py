...
# W9: per-(parent,child,bounds) path enumeration cache cap. The cap was hard-coded
# at 2048 in two places — too small at 24k where a single trace can enumerate paths
# across thousands of (caller_fn, callee_fn) pairs. Each entry is small (a list of
# path dicts), so raising the cap costs little.
import os as _os_cpp_routes  # noqa: E402
_PATHS_CACHE_MAX = max(512, int(_os_cpp_routes.environ.get("VBT_PATHS_CACHE_MAX", "16384")))

# A caller label that means "no real enclosing function was recorded".
_GLOBALISH = {"", "(global)", "global", "glob", None}

# Cross-file resolution of an entry/global symbol (not a cfg function in any file) fans out
# to every owner in the corpus entry-name map — but is skipped past this many owners, where
# the name is a truly global utility whose all-owners fan-out would over-connect the graph.
_ENTRY_FANOUT_CAP = 64
...
class CppFnGraph:
    ...
    def __init__(self, route, cfg_cache: Dict[str, List[Dict]], asm_dir: Path,
        ...
        self._paths_cache: Dict[Tuple[Tuple[str, str], Tuple[str, str], int, int],
    ...
    def _build(self) -> Dict[Tuple[str, str], Set[Tuple[str, str, int, bool]]]:
        ...
        for _i, stem in enumerate(self.stems):
            ...
            for src, tgt, line in self.route.cpp_call_edges(stem):
                ...
                if not caller:
                    continue
                local_hit = tgt in local
                if local_hit:                          # intra-file call
                    adj.setdefault((stem, caller), set()).add((stem, tgt, line, False))
                # cross-file C++ call: emit an edge to EVERY other file that defines the
                # name, not just one — and do so EVEN when the name is also local. A name
                # defined in several files is a real call fan-out the bridge walk likewise
                # explores (e.g. the parallel pipeline files dp710200 / dw710200 both
                # define processDAC + authAChargePipeline2). The old `sorted(owners)[0]`
                # tiebreak — plus the `if tgt in local: continue` short-circuit that
                # suppressed cross-file edges whenever a local same-named function existed —
                # dropped every owner but one, orphaning whole files in the graph (nothing
                # pointed into dw710200) and making fn-graph reachability under-report.
                owners = self._fn_to_file.get(tgt)     # cross-file C++ call
                if owners:
                    for callee_stem in sorted(owners):
                        if callee_stem != stem:
                            adj.setdefault((stem, caller), set()).add(
                                (callee_stem, tgt, line, True))
                if local_hit or owners:
                    continue
                # Entry/global-symbol fallback: the callee is not a recovered cfg function
                # in ANY file (e.g. DF_OK — an entry label), so _fn_to_file cannot see it.
                # Resolve it the way the bridge walk does — through the corpus entry-name map
                # — and fan out to every owner (C++ → enter at the entry name; ASM → single-
                # entry node). Capped so a truly global utility name owned by a huge number
                # of files does not over-connect the graph.
                ns_owners = self.route.name_to_stems().get(str(tgt).upper(), ()) or ()
                if ns_owners and len(ns_owners) <= _ENTRY_FANOUT_CAP:
                    for o in sorted(ns_owners):
                        if o == stem:
                            continue
                        if self._type_of(o) == "asm":
                            adj.setdefault((stem, caller), set()).add((o, o, line, True))
                        else:
                            adj.setdefault((stem, caller), set()).add((o, tgt, line, True))
                    continue
    ...
    def _build_asm_edges(
        ...
        def _dest_stems(tgt: str, entry: str) -> Set[str]:
            # Fan out to EVERY owner of the entry name, not just the file_call_graph's one
            # resolved target. An entry name (e.g. DW73) is frequently exported by several
            # files (dw730000 AND dw780000); the bridge walk reaches all of them, so the
            # old `return tgt` early-out — and the `owners[0]` single pick — silently
            # dropped the alternates and orphaned whole files in the graph.
            out: Set[str] = set()
            if tgt and tgt in self._node_types:        # the file_call_graph's resolved stem
                out.add(tgt)
            out.update(n2s.get(str(entry).upper(), ()) or ())
            if not out:                                # entry unknown → fall back to target name
                out.update(n2s.get(str(tgt).upper(), ()) or ())
            return out
        ...
        for e in self.route.graph_edges():
            ...
            for entry, ln in sites:
                for dst in _dest_stems(tgt, entry):
    ...
    def enumerate_paths(
        ...
        *, max_paths: int = 64, max_depth: int = 16,
        ...
        cache_key = (start, target, int(max_paths), int(max_depth))
        ...
        if start == target:
            out = ([[]], False)
            self._paths_cache[cache_key] = out
            return out
        ...
        def _run(order_key, budget, stale_after=None):
            ...
            nbr_cache: Dict[Tuple[str, str], Tuple[Tuple[str, str, int, bool], ...]] = {}
            path_edges: List[Dict[str, object]] = []
            visited: Set[Tuple[str, str]] = {start}

            def _neighbors(node: Tuple[str, str]) -> Tuple[Tuple[str, str, int, bool], ...]:
                cached = nbr_cache.get(node)
                if cached is None:
                    nbrs = adj.get(node, ())
                    ordered = sorted(nbrs, key=order_key) if order_key is not None else sorted(nbrs)
                    cached = tuple(ordered)
                    nbr_cache[node] = cached
                return cached
            ...
            def dfs(node):
                ...
                if len(path_edges) >= max_depth:
                    if adj.get(node):
                        st["trunc"] = True
                    return
                for (cs, cf, line, cross) in _neighbors(node):
                    nxt = (cs, cf)
                    _d = dist.get(nxt)
                    if _d is None:            # 3C: nxt can't reach target → zero-yield branch, skip
                        continue
                    if len(path_edges) + 1 + _d > max_depth:   # distance bound: shortest completion exceeds
        ...
        if not exhausted:
            ...
            if len(self._paths_cache) >= _PATHS_CACHE_MAX:
