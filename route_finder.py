...
def find_routes(
    ...
    *,
    ...
    # The child's caller forest (walking the child's full reverse closure — the dominant per-query
    # cost) is a pure function of (child, anchor, eff_depth, use_bfs), NEVER of the parent. Many
    # queries share a child: dep-var recursion checks one setter endpoint against every chain hop
    # (`any(reachable(hop, ep) for hop in chain_hops)`), and many setters share a target function.
    # So cache the forest's forward adjacency per child and reuse it across parents — only the
    # parent→child enumeration below is parent-specific. Byte-identical: `fwd`/`unverified` are
    # determined entirely by (child, anchor, eff_depth, use_bfs) + corpus; eviction only recomputes.
    fkey = (child.file_stem, child.file_type, eff_depth, use_bfs,
