...
_BUILD_TAG = "walk: bridge edge+flow index + lazy-attach + fn_graph_adj(v3 all-owners+entry-syms) + setter-map skip + db-bfs-routes + parent-prune + fn-reach-authoritative + file-reach-fallback + chain-fn-labels + prefix-guard-dest-match(issue2) + path-neighbor-cache + depvar-workers-option + dep-timing + trace-cache-visibility + setter-emit-cache + asm-full-negative-map + chain-target-setter-search-cache + skip-empty-cpp-cfg-load + real-cfg-cache-hit  [2026-06-26h]"
...
def trace_root_variable(
    ...
    *,
    ...
    if job_id and not bypass_trace_cache:
        with _Phase("setup: trace-cache lookup (first DB open)"):
            try:
                ...
                _trace_sig = None
                _log("trace-cache signature failed: %s", exc)
            if _trace_sig:
                try:
                    _cached = _TC.load(job_id, _trace_sig)
                except Exception as exc:
                    _cached = None
                    _log("trace-cache load failed sig=%s: %s", _trace_sig[:12], exc)
                else:
                    if _cached is None:
                        _log("trace-cache MISS sig=%s", _trace_sig[:12])
                    else:
    ...
    try:
        ...
        if job_id:
            ...
            try:
                ...
                _asm_full_cov = load_coverage(job_id, ASM_SETTERS_FULL_COVERAGE_ARTIFACT) or {}
                _asm_full_ok = sum(1 for _v in _asm_full_cov.values() if _v in (OK, EMPTY))
                _log(
                    "setup: asm_setters_full coverage %s (%d covered stem%s)",
                    "present" if _asm_full_cov else "absent",
                    _asm_full_ok,
                    "" if _asm_full_ok == 1 else "s",
                )
            except Exception:
                _log("setup: asm_setters_full coverage unknown")
    except Exception:
        pass
    ...
    if job_id and _trace_sig and not bypass_trace_cache:
        try:
            from vbt.precompute import trace_cache as _TC
            _cache_ok = _TC.store(job_id, _trace_sig, out)
            _log("trace-cache store %s sig=%s", "OK" if _cache_ok else "FAILED", _trace_sig[:12])
        except Exception as exc:
            _log("trace-cache store failed sig=%s: %s", _trace_sig[:12], exc)
