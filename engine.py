...
from vbt.depvars.recurse import trace_dependents, _setters_init_only
from vbt.resolve.name_resolver import resolve as resolve_aliases
from vbt.resolve.const_resolver import get_const_resolver, iter_resolvable_tokens
...
_BUILD_TAG = "walk: bridge edge+flow index + lazy-attach + fn_graph_adj(v3 all-owners+entry-syms+slice) + setter-map skip + db-bfs-routes + parent-prune + fn-reach-authoritative + file-reach-fallback + chain-fn-labels + prefix-guard-dest-match(issue2) + path-neighbor-cache + depvar-workers-option + depvar-direct-cpp-asm-v3-keyset + depvar-lazy-cfg + loadonly-cond-cache + large-trace-cache-skip + empty-path-family-skip + depvar-cross-chain-node-cache + const-token-filter + icm-memory-form + scratch-local-prefixes + lineage-schema-v2  [2026-07-03c]"
...
def _merge_chain(prefix: List[Tuple[str, str]], path: List[Tuple[str, str]]) -> List[str]:
    """Order-preserving union of ``(stem, formatted_label)`` hops.

    GAP 8: a file may appear MORE THAN ONCE — once per distinct intra-file function the
    route enters (``stem::fnA`` then ``stem::fnB``) — so dedup is BY LABEL, not by stem.
    Two invariants from Issue 1 / TC-01 are preserved, so this stays a display-only change:
      * bare<->qualified collapse: a bare ``stem`` hop and a qualified ``stem::fn`` hop for
        the SAME file are one node. A bare placeholder is upgraded IN PLACE to the first
        function learned for that file (the entry hop); a bare hop arriving after the file
        is already qualified is dropped. (P1)
      * no downgrade: a qualified label is never replaced by a later bare stem. (P5)
    Returns a plain list of label strings."""
    out: List[str] = []
    label_at: Dict[str, int] = {}      # exact label -> index in `out` (P2 exact-dup guard)
    bare_at: Dict[str, int] = {}       # stem -> index of a bare placeholder awaiting a function
    qualified: Set[str] = set()        # stems with >=1 qualified hop already in `out`
    for stem, label in prefix + path:
        if label in label_at:
            continue                                   # P2: exact duplicate hop
        if label == stem:                              # bare hop
            if stem in bare_at or stem in qualified:
                continue                               # P1: file already represented
            bare_at[stem] = len(out)
            label_at[label] = len(out)
            out.append(label)
        else:                                          # qualified hop ``stem::fn``
            if stem in bare_at:                        # upgrade the placeholder in place
                i = bare_at.pop(stem)
                del label_at[stem]                     # drop the stale bare-label index
                out[i] = label
                label_at[label] = i
            else:                                      # P3: a new distinct function for this file
                label_at[label] = len(out)
                out.append(label)
            qualified.add(stem)
    return out
...
def _chain_richness(chain: List[str]) -> Tuple[int, int]:
    """Sort key for choosing the most informative chain among entries that collapse to the
    same file-path identity: more hops first, then more function-qualified hops. A genuine
    tie (equal length and qualification) keeps the first-enumerated chain — deterministic,
    since the path enumerator is deterministic."""
    return (len(chain), sum(1 for h in chain if "::" in h))


def _route_path_chain(rt: Dict[str, Any], *, tail: Hop, setter: SetterSite,
                      route: RouteEngine, cfg_cache: Dict[str, List[Dict]], asm_dir: Path,
                      max_paths: int = 64, max_call_depth: int = 16
                      ) -> Tuple[List[Tuple[str, str]], bool]:
    """Build the ``(stem, formatted_label)`` hop list for an ASM/mixed route (TC-01).

    A mixed route's ``rt["files"]`` is file-level, but ``rt["hops"]`` carries the
    function entered at each cross-file edge via ``to_function`` (route_finder's
    ``site_target_fn``, which is non-empty ONLY for a C++ target — cpp→cpp
    ``callee_function`` / asm→cpp ``cpp_function``). So a C++ hop on a mixed route
    can be qualified ``stem::fn`` exactly like the pure-C++ branch, while ASM hops
    (single-entry modules) stay bare stems. OUTPUT FORMAT ONLY: ``_chain_label`` is
    the sole gate on whether a function shows, and every internal check stays
    file-stem based (see ``_chain_stem`` / ``chain_union``). Never invents a label.

    GAP 8: for each C++ file on the route we also inline the intra-file call path from
    the function ENTERED (``to_function``/``tail.function``) to the function that issues
    the next cross-call (the departing hop's ``from_function``, or ``setter.function`` for
    the setter file), using the SAME ``intra_call_paths`` primitive the pure-C++ branch
    uses. We inline the RICHEST (longest) path so coverage grows without growing the entry
    count (Option A). ASM files stay single bare stems (no intra-file call graph yet).
    Returns ``(hops, truncated)`` — ``truncated`` is True if any segment hit a path cap, so
    the caller can keep ``pathsCapped`` honest on mixed routes."""
    rt_files = rt.get("files") or [setter.file_stem]
    hops = rt.get("hops") or []
    # Prefer the already-populated type map; fall back to node_types() for a preloaded
    # route-cache hit that returns before _ensure_graph() ran. Graph/index metadata
    # only — no per-hop source-file probes.
    try:
        node_types = route._node_type or route.node_types()
    except Exception:
        node_types = route._node_type or {}
    out: List[Tuple[str, str]] = []
    intra_truncated = False

    for i, stem in enumerate(rt_files):
        ftype = node_types.get(stem)
        fn: Optional[str] = None

        if i == 0:
            # The first file is the route's parent — i.e. the chain tail; use the
            # tail's own authoritative type/function (mirrors the pure-C++ branch).
            if stem == tail.stem:
                ftype = ftype or tail.file_type
                fn = tail.function
            elif hops and hops[0].get("from_file") == stem:
                fn = hops[0].get("from_function")
        else:
            # Align by route-file index first, then fall back to to_file matching
            # (robust to a malformed/reordered hop list — no behaviour change normally).
            hop = hops[i - 1] if i - 1 < len(hops) else None
            if not hop or hop.get("to_file") != stem:
                hop = next((h for h in hops if h.get("to_file") == stem), None)
            if hop:
                fn = hop.get("to_function")
                # to_function is non-empty only for a C++ target, so a non-empty fn
                # with no recorded node type can be safely typed C++.
                if fn and not ftype:
                    ftype = "cpp"

        if stem == setter.file_stem:
            # The setter file: the engine already has authoritative setter metadata.
            ftype = ftype or setter.language
            fn = fn or setter.function

        out.append((stem, _chain_label(stem, ftype, fn)))

        # GAP 8: inline the intra-file C++ call path entry_fn -> exit_fn for this segment.
        if ftype == "cpp" and fn and (asm_dir / f"{stem}.cpp").exists():
            if stem == setter.file_stem:
                exit_fn = setter.function
            else:
                dep = next((h for h in hops if h.get("from_file") == stem), None)
                exit_fn = (dep or {}).get("from_function")
            if exit_fn and exit_fn != fn:
                edges = route.cpp_call_edges(stem)
                local = _local_fn_names(_cfg_for(stem, cfg_cache, asm_dir))
                paths, trunc = intra_call_paths(edges, local, fn, exit_fn,
                                                max_paths=max_paths, max_depth=max_call_depth)
                intra_truncated = intra_truncated or trunc
                best = max(paths, key=len, default=[])     # richest; deterministic on ties
                out += [(stem, _chain_label(stem, "cpp", callee)) for (_c, _l, callee) in best]

    return out, intra_truncated
...
# --------------------------------------------------------------------------- #
# GAP 5: empty synthetic path-family ancestor hygiene (root-trace output only)
# --------------------------------------------------------------------------- #
# A qualified member-path dep (``a.b.c``) makes ``trace_dependents`` emit synthetic
# CONTAINER-ANCESTOR nodes (``a.b``, ``a``) so an aggregate write to a parent object
# (``memcpy(&a.b, …)``) is still found. When such an ancestor actually carries a
# setter (or other surfaced evidence) it is real lineage and stays. But an ancestor
# with NO setters, terminal, and existing only to link the path-family tree adds no
# dataflow and no business meaning — at scale it is pure token/UI noise. We drop ONLY
# those empty helper ancestors from the FINAL root output (not from the compute layer:
# ``trace_dependents`` still emits + links them, which the recursion tests assert).
#
# Flags that mean "this node carries something a consumer must see" — never drop a
# node carrying one (a capped/fallback/diagnostic ancestor stays so the truncation is
# visible). ``aliases`` is checked separately (it is a list, not a truthy flag here).
_PATH_FAMILY_KEEP_FLAGS = (
    "depthCapped",
    "offchainSearchCapped",
    "descendantsCapped",
    "functionBodyUnresolved",
    "externalCall",
    "byRefOutputs",
    "indirection",
)
...
def _filter_empty_path_family_ancestors(dep_objs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Suppress empty synthetic path-family ANCESTOR nodes from the root trace's
    ``dependentVariables`` and repair the surviving nodes' ``pathParents`` /
    ``pathChildren`` links so they no longer reference a removed path.

    A node is removable iff ALL of: ``origin == "ancestor"``; it is referenced as a
    path-family parent/child helper (its ``qualified`` appears in some node's
    ``pathParents``/``pathChildren``); ``terminal is True``; no ``setters``; no
    ``aliases``; and it carries none of ``_PATH_FAMILY_KEEP_FLAGS``. This keeps primary
    terminal inputs (origin != "ancestor"), aggregate-setter ancestors, concrete
    sub-field descendants, and any capped/diagnostic node. Order of survivors is
    preserved; the original list is returned unchanged when nothing is removable.

    Mutates link fields on surviving entries in place — sound because ``dep_objs`` is
    freshly computed in this trace and has no external owner before ``build_root_output``."""
    if not dep_objs:
        return dep_objs

    # Every qualified path referenced as a path-family link by ANY node (computed over
    # the original, pre-removal set — that is what "was linked" means).
    linked_quals: set = set()
    for entry in dep_objs:
        dv = entry.get("dependentVariable") or {}
        linked_quals.update(q for q in (dv.get("pathParents") or []) if q)
        linked_quals.update(q for q in (dv.get("pathChildren") or []) if q)

    removable: set = set()
    for entry in dep_objs:
        dv = entry.get("dependentVariable") or {}
        qual = dv.get("qualified") or ""
        if not qual:
            continue
        if dv.get("origin") != "ancestor":
            continue
        if qual not in linked_quals:
            continue
        if dv.get("terminal") is not True:
            continue
        if dv.get("setters"):
            continue
        if dv.get("aliases"):
            continue
        if any(dv.get(flag) for flag in _PATH_FAMILY_KEEP_FLAGS):
            continue
        removable.add(qual)
    ...
    for entry in dep_objs:
        dv = entry.get("dependentVariable") or {}
        if (dv.get("qualified") or "") in removable:
            continue
        for link_key in ("pathParents", "pathChildren"):
            if link_key in dv:
                kept_links = [q for q in dv.get(link_key, []) if q not in removable]
                if kept_links:
                    dv[link_key] = kept_links
                else:
                    dv.pop(link_key, None)
...
def trace_root_variable(
    ...
    *,
    ...
    emit_not_set: bool = True,       # GAP 9: emit "[not set]" outcomes (¬ of the setters' guards); False ⇒ byte-identical to pre-GAP-9
    emit_indirect_writes: bool = False,  # GAP 6: surface the C++→ASM register-bridge data source (`«SL70»`) under its guard; OFF ⇒ byte-identical to HEAD
    scratch_local_prefixes: Tuple[str, ...] = (),  # ASM scratch work-field prefixes searched module-locally (recurse._is_scratch_local); () ⇒ unchanged
    depvar_workers: Optional[int] = None,  # route-warm workers for dep-var expansion; None => VBT_DEPVAR_WORKERS/default
    ...
    _shared_dep_node_cache: Optional[Any] = None,  # cross-chain dep-var node cache (vbt.depvars.recurse.DepNodeCache):
                                                   # validated per-node memo — chains 2..N of the SAME variable reuse
                                                   # chain 1's dep-var subtree computes wherever the chain-scope
                                                   # revalidation proves the output identical. Byte-identical per chain.
    ...
    # GAP 6: the indirect-writes feature is opt-in via the param OR the VBT_INDIRECT_WRITES env
    # (so it can be flipped on for a corpus run without a code change). Default OFF ⇒ byte-identical.
    if os.environ.get("VBT_INDIRECT_WRITES"):
        emit_indirect_writes = True
        ...
        with _Phase("setup: trace-cache lookup (first DB open)"):
            try:
                ...
                _trace_sig = _TC.signature(
                    ...
                    asm_max_levels=asm_max_levels, emit_not_set=emit_not_set,
                    emit_indirect_writes=emit_indirect_writes,
    ...
    store = CodeBlockStore(asm_dir, shared_src=_shared_src, job_id=job_id)
    ...
    try:
        ...
        from vbt.precompute.cpp_indirect_writes_db import set_cpp_indirect_writes_job, clear_caches as _clear_iw_caches
        ...
        set_cpp_indirect_writes_job(None)  # GAP 6: clear any prior trace's indirect-writes hook
        _clear_iw_caches()                 # GAP 6: drop a prior trace's live/DB bridge caches
        if job_id:
            ...
            set_cpp_indirect_writes_job(job_id)  # GAP 6: serve C++→ASM bridge writers from the precomputed map
    except Exception:
        pass
    ...
    # ---- GAP 6: merge C++→ASM register-bridge setters when the ROOT var is an init-only buffer ----
    # The data source (`«SL70»` / `«retrieveCrcnCidData»`) surfaces directly on the root, under its
    # VPAY guard. Scoped to the root's writer/chain files (a stack buffer is local to its TU). Added
    # BEFORE not-set so the not-set complement is over the real setters only. Gated by
    # emit_indirect_writes (OFF ⇒ skipped, byte-identical). Bridge setters are LEAVES (per-setter loop).
    if emit_indirect_writes and language == "cpp" and _setters_init_only(setters):
        from vbt.precompute.cpp_indirect_writes_db import load_bridge_writers
        _tail0 = variable.split(".")[-1].split("->")[-1]
        _b_stems = sorted(set(chain_stem_set) | {s.file_stem for s in setters if s.language == "cpp"})
        _seen_b: set = set()
        n_bridge = 0
        for _bstem in _b_stems:
            for _bs in load_bridge_writers(job_id, _bstem, _tail0, asm_dir=asm_dir,
                                           blueprint_dir=blueprint_dir):
                _bk = (_bs.file_stem, _bs.line, str(_bs.value))
                if _bk not in _seen_b:
                    _seen_b.add(_bk)
                    setters.append(_bs)
                    n_bridge += 1
        if n_bridge:
            _log("root indirect-writes merge (GAP 6): %d bridge setters added", n_bridge)

    # ---- GAP 9: inject "[not set]" virtual setters (¬ of the real setters' guards) ----
    # For each C++ function / ASM module that conditionally writes V, add ONE virtual setter for the
    # paths on which V is NOT written — its guard is the logical complement of that scope's real
    # setters' reaching conditions, attached via preset_conditions so the per-setter loop uses it
    # verbatim instead of recomputing at the (synthetic) line. The virtual setter shares the
    # function/module with the real setters, so route discovery / reachability already succeed and
    # the per-route loop scopes each not-set entry to its route. Gated by emit_not_set (False ⇒
    # byte-identical to pre-GAP-9). Trace-time only: never enter the precomputed setter map.
    if emit_not_set:
        with _Phase("not-set injection (GAP 9)"):
            def _ns_asm_bp(_stem):
                _bp = _asm_bp(_stem, blueprint_dir, asm_bp_cache)
                if not _bp:
                    return None, None
                try:
                    return str(resolve_asm_blueprint(_stem, blueprint_dir)), _bp
                except Exception:
                    return None, None
            _ns_virtual = build_not_set_setters(
                setters, asm_dir=asm_dir,
                get_cpp_fns=lambda _st: _cfg_for(_st, cfg_cache, asm_dir),
                get_asm_bp=_ns_asm_bp, asm_max_levels=asm_max_levels, log=_log)
            setters.extend(_ns_virtual)
            _log("not-set injection: %d virtual setters added", len(_ns_virtual))
    ...
    seen_tuples: Dict[tuple, int] = {}         # tkey -> index in `entries` (prefer-richest chain)
    ...
    for _si, s in enumerate(setters):
        ...
        if s.language == "cpp":
            ...
            # GAP 9: a not-set virtual setter carries its guards (the negation of the real setters'
            # reaching conditions) verbatim — they correspond to no single line, so they cannot be
            # recomputed by collect_cpp_conditions. Real setters keep preset_conditions=None ⇒ unchanged.
            local = (s.preset_conditions if s.preset_conditions is not None
                     else collect_cpp_conditions(asm_dir / f"{s.file_stem}.cpp", s.line, s.function, functions=fns))
            ...
            asm_ind_ctx = AsmIndirectContext(bp_data=bp, bp_path=bp_path, asm_dir=asm_dir,
                                             route_engine=route)
            if s.preset_conditions is not None:
                # GAP 9: a not-set ASM setter carries its (negated) guards verbatim — never
                # recompute at the synthetic line (which would re-collect the REAL guard).
                local, local_trunc = s.preset_conditions, False
            else:
        ...
        if via_unverified:
            ...
            # GAP 8: the one case where verified intra-file structure exists is a setter in
            # the TAIL FILE itself — its entry is the verified tail.function, so reconstruct
            # that intra-file path (anything else would fabricate an unverified hop). Else
            # keep the single function-qualified hop (the honest ceiling — the cross-file
            # gap is already flagged by reachableOnlyViaUnverified).
            if (s.language == "cpp" and s.file_stem == tail.stem and tail.function
                    and (asm_dir / f"{s.file_stem}.cpp").exists()):
                edges = route.cpp_call_edges(s.file_stem)
                local_fns = _local_fn_names(_cfg_for(s.file_stem, cfg_cache, asm_dir))
                paths, _ = intra_call_paths(edges, local_fns, tail.function, s.function or "",
                                            max_paths=max_paths, max_depth=max_call_depth)
                best = max(paths, key=len, default=[])
                path_chain = [(s.file_stem, _chain_label(s.file_stem, "cpp", tail.function))]
                path_chain += [(s.file_stem, _chain_label(s.file_stem, "cpp", c))
                               for (_a, _b, c) in best]
            else:
        ...
        for rt in routes:
            ...
            if rt_pure_cpp:
                ...
                for path in paths:
                    ...
                    # (stem, formatted_label): the C++ tail + each function entered on the
                    # path. GAP 8: intra-file hops (cross=False) are now KEPT so the chain
                    # shows ``stem::fnA -> stem::fnB`` instead of collapsing the tail file to
                    # its entry function; _merge_chain dedups bare<->qualified per file.
                ...
                raw += [(c, blk, None) for c in local]
                # TC-01: rt["hops"] carries the function entered at each cross-file edge
                # (to_function), so a C++ hop on this mixed route is qualified ``stem::fn``
                # (matching the pure-C++ branch); ASM hops stay bare stems. Display-only:
                # the chain still dedupes/scopes by stem (route_fns stays empty here, so
                # the value-flow prune remains a no-op on ASM/mixed routes).
                path_chain, intra_trunc = _route_path_chain(
                    rt, tail=tail, setter=s, route=route, cfg_cache=cfg_cache, asm_dir=asm_dir,
                    max_paths=max_paths, max_call_depth=max_call_depth)
                built.append((path_chain, raw,
                              routes_capped or cpp_paths_trunc or local_trunc or intra_trunc, []))
        ...
        for path in cpp_paths:
            ...
            for e in path:
                ...
                for c in _edge_guard(e, cfg_cache, asm_dir):
                    raw.append((c, eblk, via))
            raw += [(c, blk, None) for c in local]
            # GAP 8: keep intra-file hops (cross=False) too, so the chain surfaces every
            # function entered on the path (``stem::fnA -> stem::fnB``); _merge_chain dedups
            # bare<->qualified per file. route_fns (value-flow prune input) is unchanged.
            path_chain = [(tail.stem, _chain_label(tail.stem, tail.file_type, tail.function))]
            path_chain += [
                (e["callee_stem"], _chain_label(e["callee_stem"], "cpp", e.get("callee_fn")))
                for e in path
        ...
        for path_chain, raw, incomplete, route_fns in built:
            ...
            if _route_value_flow_infeasible(conditions, route_fns, cfg_cache, asm_dir,
                ...
                continue
            # GAP 9: a not-set entry is a LEAF — its scrutinee is visible in the condition text,
            # but it does NOT spawn dependent-variable nodes/edges. Recursing it would expand the
            # shared dep tree (and, via multi-read-site rescope, even alter a real node's setters),
            # breaking the feature's strictly-additive guarantee. So no dep-vars / no seeding.
            # GAP 6: a bridge setter (`«SL70»`) is likewise a LEAF (level A) — the «module» token is
            # not a C++ expression to recurse; its data source is named in value/dataSource.
            _is_bridge = getattr(s, "bridge", None) is not None
            dep_refs = [] if (s.is_not_set or _is_bridge) else extract_dep_vars(
                s, kept, constant_symbols=consts,
            ...
            # GAP 8: intra-file hops add DUPLICATE stems to the displayed chain, but they must
            # NOT mint new entries (a display enrichment, not a new routing dimension) — so
            # the stem component is the UNIQUE stems in order (restores the pre-GAP-8 key).
            # GAP 9: is_not_set discriminates a not-set entry from a real setter that might
            # coincidentally share (file, line, chain, conditions) — they are distinct outcomes.
            tkey = (setter_loc["file"], setter_loc["startLine"],
                    tuple(dict.fromkeys(_chain_stem(f) for f in chain)),
                    tuple(sorted(c["condition"] for c in conditions)), via_sig, s.is_not_set)
            prev = seen_tuples.get(tkey)
            if prev is not None:
                # Same setter, same file-path, same guards/via → same entry. Keep the
                # RICHEST chain (most hops, then most function-qualified) so the intra-file
                # flow shown is deterministic and maximal regardless of enumeration order.
                # Everything else (conditions, deps, flags) is identical by the tkey, so
                # swapping only `chain` keeps the entry self-consistent; the unique-stem
                # sequence is in the tkey, so chain_union (dep-var seed) is unchanged.
                if _chain_richness(chain) > _chain_richness(entries[prev]["chain"]):
                    entries[prev]["chain"] = chain
                continue
            seen_tuples[tkey] = len(entries)
            entry = {
                "value": s.value,
                # GAP 9 / GAP 6: "[not set]" and `«module»` bridge tokens are not expressions → no resolved value.
                "valueResolved": None if (s.is_not_set or _is_bridge) else cr.resolve(s.value or "", s.language),
                "setterCodeChunk": s.setter_code_chunk,
                "location": setter_loc,
                # GAP 6: the setter-local code block id (already computed above) so the
                # assembler can derive a C++ ``function`` and consumers get a direct
                # blockId reference. ASM blk is ``asm:{stem}:{block_id}`` → no function.
                "blockId": blk,
                "chain": chain,
            ...
            if s.is_not_set:
                # GAP 9: a not-set entry always carries ≥1 guard (the complement of a non-empty
                # set of setter guards); never "unconditional". Surface the marker for consumers.
                assert conditions, "not-set entry must carry conditions"
                entry["isNotSet"] = True
            if _is_bridge:
                # GAP 6: surface the resolved cross-language data source (ASM module / external callee).
                entry["dataSource"] = dict(s.bridge)
            if incomplete:
    ...
    if disable_dependents:
        ...
        with _Phase("dependent expansion"):
            ...
            if len(seen_seed) > 25:
                _log("  dep vars: %d unique seeds (%d shown)", len(seen_seed), 25)
            ...
            dep_objs = trace_dependents(
                ...
                workers=depvar_workers,
                progress=progress,
                emit_not_set=emit_not_set,
                emit_indirect_writes=emit_indirect_writes,
                suppress_empty_path_family_ancestors=True,
                job_id=job_id,
                node_cache=_shared_dep_node_cache,
                scratch_local_prefixes=scratch_local_prefixes,
            ) if seed_deps else []
            # GAP 5: drop empty synthetic path-family ancestor helper nodes from the
            # FINAL output (compute layer still emits + links them). Output hygiene only.
            _before_filter = len(dep_objs)
            dep_objs = _filter_empty_path_family_ancestors(dep_objs)
            if len(dep_objs) != _before_filter:
                _log("dependent expansion: filtered empty path-family ancestors: %d -> %d",
                     _before_filter, len(dep_objs))
            _log("dependent expansion: %d dependent variables", len(dep_objs))

    out = build_root_output(
        variable, entries, store, dependent_variables=dep_objs, normalize_deps=False)
