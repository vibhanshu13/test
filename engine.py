...
_BUILD_TAG = "walk: bridge edge+flow index + lazy-attach + fn_graph_adj(v3 all-owners+entry-syms) + setter-map skip + db-bfs-routes + parent-prune + fn-reach-authoritative + file-reach-fallback + chain-fn-labels + prefix-guard-dest-match(issue2) + path-neighbor-cache + depvar-workers-option + dep-timing + trace-cache-visibility + setter-emit-cache + asm-full-negative-map + chain-target-setter-search-cache  [2026-06-26f]"
