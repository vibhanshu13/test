...
def precompute_vbt_db(job_id: str, blueprint_dir, asm_dir, graph_file=None,
    ...
    # ---- per-substep timing instrumentation ---------------------------------- #
    # Lightweight: prints "  [VDB] <name>: <secs>s" so the wave deltas are visible
    # in job.log without any other source change. No effect on output.
    import time as _vdb_time
    _vdb_timings: Dict[str, float] = {}

    def _vdb(name: str, fn, *a, **kw):
        _t0 = _vdb_time.monotonic()
        try:
            return fn(*a, **kw)
        finally:
            _elapsed = _vdb_time.monotonic() - _t0
            _vdb_timings[name] = round(_elapsed, 3)
            print(f"  [VDB] {name}: {_elapsed:.2f}s", flush=True)
    ...
    _vdb("ensure_call_graph", ensure_call_graph, bp, src, out_path=gf)
    ...
    out["resolvers"] = bool(_vdb("preload_resolvers", preload_resolvers, job_id, bp, src))
    ...
    out["file_call_graph"] = _vdb("file_call_graph", build_and_store_file_call_graph, job_id, bp, gf)

    # Route name_to_stems (build+persist) + per-stem cpp_call_edges (build+store).
    route = RouteEngine(bp, gf, src, job_id=job_id)
    _vdb("preload_route_graph", preload_route_graph, route, job_id, bp, gf)
    out["cpp_call_edges"] = len(_vdb("cpp_call_edges", build_and_store_cpp_call_edges, job_id, bp, src, gf))
    ...
    out["reverse_adj"] = _vdb("reverse_adj", build_and_store_reverse_adj, job_id, bp, gf)
    out["forward_file_adj"] = _vdb("forward_file_adj", build_and_store_forward_file_adj, job_id, route)

    # ASM blueprints + raw source text.
    from vbt.precompute.asm_db import build_and_store_asm_blueprints
    out["asm_blueprints"] = _vdb("asm_blueprints", build_and_store_asm_blueprints, job_id, bp)
    from vbt.precompute.source_db import build_and_store_source
    out["source"] = _vdb("source", build_and_store_source, job_id, src)
    # GAP 7: the comment-stripped C/C++ variant the output code-block slicer reads.
    from vbt.precompute.source_nocomments_db import build_and_store_source_nocomments
    out["source_nocomments"] = _vdb(
        "source_nocomments", build_and_store_source_nocomments, job_id, src)
    ...
    out["asm_setter_map"] = _vdb("asm_setter_map", build_and_store_asm_setter_map, job_id, bp, src)
    from vbt.precompute.cpp_setter_map_db import build_and_store_cpp_setter_map
    out["cpp_setter_map"] = _vdb("cpp_setter_map", build_and_store_cpp_setter_map, job_id, bp, src)
    ...
    out["cpp_file_writes"] = _vdb("cpp_file_writes", build_and_store_cpp_file_writes, job_id, bp, src)
    from vbt.precompute.fn_facts_db import build_and_store_fn_facts
    out["fn_facts"] = _vdb("fn_facts", build_and_store_fn_facts, job_id, bp, src)
    ...
    out["fn_graph_adj"] = _vdb("fn_graph_adj", build_and_store_fn_graph_adj, job_id, route, src)

    # cfg blobs: final DB population pass after fn_graph_adj.
    out.update(_vdb("cfg_blobs", build_and_store_cfg_blobs, job_id, src))
    # Direct dep-var relations: C++ tail -> setter sites, C++ callee tail -> call sites,
    # and ASM variable -> direct setter sites with coverage.
    # This is intentionally NOT a transitive/deep precompute; trace-time still applies
    # reachability, chain scoping, and recursion limits.
    from vbt.precompute.depvar_direct_db import build_and_store_depvar_direct
    out["depvar_direct"] = _vdb("depvar_direct", build_and_store_depvar_direct, job_id, bp, src)
    # cpp_indirect_writes (GAP 6): C++→ASM register-bridge data-source setters + the corpus-global
    # binding map. Runs AFTER cfg_blobs so its guard collection reuses the just-built CFG via the DB
    # (install_cfg_db) — no second cfg_extract. A bounded tree walk per file (a small fraction of cpp_file_writes).
    from vbt.precompute.cpp_indirect_writes_db import build_and_store_cpp_indirect_writes
    out["cpp_indirect_writes"] = _vdb("cpp_indirect_writes", build_and_store_cpp_indirect_writes, job_id, bp, src)
    out["_vdb_timings"] = _vdb_timings
...
def build_only(job_id: str, blueprint_dir, asm_dir, graph_file=None, names=None) -> Dict[str, object]:
    ...
    def _resolvers():
        # Builds + persists name_alias + membership + const + modifier_index in one call
        # (load_only is OFF here, so preload_resolvers goes through the build+persist path).
        # The full precompute is the only OTHER place these blobs are built (precompute_vbt_db);
        # this lets a system that already ran a full precompute add the resolver blobs
        # incrementally — the fix for the §1.3 "name_alias absent/empty" storm.
        from vbt.precompute.resolvers_db import preload_resolvers
        return bool(preload_resolvers(job_id, bp, src))

    def _source_nocomments():
        # GAP 7: rebuild the comment-stripped C/C++ variant in lockstep with `source`.
        from vbt.precompute.source_nocomments_db import build_and_store_source_nocomments
        return build_and_store_source_nocomments(job_id, src)

    def _cpp_indirect_writes():
        # GAP 6: C++→ASM register-bridge data-source setters. Requires cfg_blobs already built
        # (guard collection reuses the cfg via the DB), so run after a full precompute or a `cfg` build.
        from vbt.precompute.cpp_indirect_writes_db import build_and_store_cpp_indirect_writes
        return build_and_store_cpp_indirect_writes(job_id, bp, src)

    def _depvar_direct():
        from vbt.precompute.depvar_direct_db import build_and_store_depvar_direct
        return build_and_store_depvar_direct(job_id, bp, src)

    def _chain_facts():
        # Chains precompute (pipeline Step 3.5): asm_setters_full + both setter-map coverages +
        # the per-edge chain_facts. Lets a KB whose pipeline died mid-3.5 resume via the repair
        # API instead of a full re-run. Requires the core precompute artifacts (fn_graph_adj,
        # setter maps, cfg) to already exist.
        from vbt.precompute.chain_facts_db import build_and_store_all
        return build_and_store_all(job_id, bp, src, gf)
    ...
    REGISTRY = {
        ...
        "cfg": _cfg,
        "resolvers": _resolvers,   # name_alias + membership + const + modifier_index
        "source_nocomments": _source_nocomments,   # GAP 7: comment-stripped C/C++ variant
        "cpp_indirect_writes": _cpp_indirect_writes,   # GAP 6: C++→ASM register-bridge writers
        "depvar_direct": _depvar_direct,   # direct C++/ASM dep-var setter/callsite relations
        "chain_facts": _chain_facts,       # chains precompute (Step 3.5): setters_full + coverages + edge facts
