...
def _resolve_asm_dir(job_id: str, options: Dict[str, Any]) -> Path:
    ...
    recorded source_path for the job — descended to the directory that actually
    holds the sources (the stored path may be the datasource root with the files
    one level deeper, e.g. an extracted zip; same resolution the pipeline uses).
    Building against the bare root silently finds NO source and produces empty
    guard/setter artifacts."""
    ...
    try:
        ...
        if sp:
            ...
            return _find_source_dir(Path(sp))
    except Exception:
        pass
    raise ValueError(
...
VBT_DB_REPAIR_ARTIFACTS: Set[str] = {
    ...
    "cfg",
    "depvar_direct",
    "chain_facts",     # chains precompute (pipeline Step 3.5): setters_full + coverages + edge facts
    "lineage_scans",   # the three cached /vbt/lineage corpus scans (io markers, entry aliases, audit)
}
...
def run_db_repair(
    ...
    # lineage_scans is an api-layer artifact (lives in vbt_lineage_task, not the vbt registry):
    # peel it off before build_only and run it afterwards, so `only: ["chain_facts",
    # "lineage_scans"]` reproduces pipeline Step 3.5 exactly.
    run_lineage_scans = "lineage_scans" in names
    vbt_names = [n for n in names if n != "lineage_scans"]

    with _scoped_env({"VBT_CACHE_DIR": env_cache}):
        res = build_only(job_id, blueprint_dir, asm_dir, graph_file, names=vbt_names) if vbt_names else {}
        if run_lineage_scans:
            from api.tasks.vbt_lineage_task import warm_lineage_scans
            res["lineage_scans"] = warm_lineage_scans(job_id)
...
# --------------------------------------------------------------------------- #
def _parse_warm_specs(raw: Optional[List[Dict[str, Any]]]) -> Optional[List[Dict[str, Any]]]:
    """Normalize API-supplied eager-warm trace specs for ``precompute_vbt_db(warm=...)``.

    Each spec's ``hops`` entries may be dicts ``{stem, type, function}``, strings
    ``"stem:type[:function]"``, or [stem, type, function] lists — all become ``Hop``
    tuples (TAIL last). Specs without a variable or hops are dropped with a log line
    (warming is best-effort by contract; it must never fail the precompute)."""
    if not raw:
        return None
    ...
    for w in raw:
        if not isinstance(w, dict):
            continue
        hops = []
        for h in (w.get("hops") or []):
            if isinstance(h, str):
                parts = h.split(":")
                hops.append(Hop(parts[0], parts[1] if len(parts) > 1 else "asm",
                                parts[2] if len(parts) > 2 else None))
            elif isinstance(h, dict):
                hops.append(Hop(h.get("stem"), h.get("type", "asm"), h.get("function")))
            elif isinstance(h, (list, tuple)) and h:
                hops.append(Hop(h[0], h[1] if len(h) > 1 else "asm",
                                h[2] if len(h) > 2 else None))
        if not w.get("variable") or not hops:
            logger.warning("[vbt_precompute] dropping invalid warm spec (needs variable + hops): %r", w)
            continue
        specs.append({**w, "hops": hops, "language": w.get("language", "cpp")})
...
def run_precompute(
    ...
    ``options``::
        ...
        warm                optional list of eager-warm trace specs (see
                            _parse_warm_specs) — runs real traces after the build
                            to fill the route + trace caches. Best-effort.
    """
    ...
    warm_specs = _parse_warm_specs(options.get("warm"))
    ...
    with _scoped_env({"VBT_CACHE_DIR": env_cache, "VBT_WORKERS": env_workers}):
        ...
        try:
            ...
            _t = time.monotonic()
            db_res = precompute_vbt_db(job_id, blueprint_dir, asm_dir, gf, warm=warm_specs)
            _e = time.monotonic() - _t
