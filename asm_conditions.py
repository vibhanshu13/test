...
_ASM_COND_CACHE: Dict[Tuple[str, str, int, str, int], Tuple[List[Condition], bool]] = {}
_ASM_COND_CACHE_MAX = 100_000


def _asm_cond_cache_put(key, value) -> None:
    if len(_ASM_COND_CACHE) >= _ASM_COND_CACHE_MAX:
        _ASM_COND_CACHE.pop(next(iter(_ASM_COND_CACHE)))
    _ASM_COND_CACHE[key] = value
...
def collect_asm_conditions(
    ...
    *,
    ...
    """
    _ck = (str(bp_path), str(block_id), int(before_line or 0), str(file), int(max_levels))
    _hit = _ASM_COND_CACHE.get(_ck)
    if _hit is not None:
        conds, trunc = _hit
        return copy.copy(conds), trunc
        ...
        for p in dom[block_id]:
            if p in block_map and p != block_id:
                if depth.get(p, max_levels + 1) <= max_levels:
                    ...
                    truncated = True
        domset = sorted(domset_candidates, key=lambda p: depth.get(p, 1 << 30))
        for P in domset:
    ...
    for i, c in enumerate(conditions, start=1):
        c.order = i
    _asm_cond_cache_put(_ck, (copy.copy(conditions), truncated))
