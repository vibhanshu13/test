        ...
        if db_fast_path:
            ...
            try:
                ...
                _hs = health_summary(
                    ...
                    blueprint_dir=None,
                    asm_dir=None,
                    require_speedup_artifacts=False,
                )
                _warns = list(_hs.get("missing") or []) + list(_hs.get("warnings") or [])
                if _warns:
                    _counts = _hs.get("counts") or {}
                    logger.warning(
                        "[vbt_trace] job=%s DB fast path has missing speedup artifacts for %s: %s counts=%s",
                        job_id, precompute_job_id, _warns, _counts,
                    )
                    print(
                        "  [WARN] DB speedup artifacts incomplete: "
                        f"{', '.join(map(str, _warns))}",
                        flush=True,
                    )
            except Exception as exc:
                logger.debug("[vbt_trace] speedup health check skipped: %s", exc)
            print(
