    ...
    def reachable(self, parent: Endpoint, child: Endpoint) -> bool:
        if self.job_id:
            ...
            try:
                ...
                parent_distances = None
            try:
                ok = is_reachable_bounded(
                    parent, child,
                    blueprint_dir=self.blueprint_dir, asm_dir=self.asm_dir,
                    max_len=self.max_len,
                    reverse_adj=self._reverse_adj or {}, node_type=self._node_type or {},
                    parent_distances=parent_distances,
                    _pfl_cache=self._pfl_cache, reach_cache=self._reach_cache,
                )
            except Exception:
                ok = bool(self.routes(parent, child, max_routes=1).get("reachable"))
