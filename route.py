...
class RouteEngine:
    ...
    def reachable(self, parent: Endpoint, child: Endpoint) -> bool:
        if self.job_id:
            ...
            if cached is not None:
                return cached
            # SOUND POSITIVE fast-accept only. The unified fn-graph is a STRICT SUBSET of
            # true reachability (it drops links wherever a cfg was skipped, a cross-file
            # target name did not resolve to a known function, or caller attribution
            # failed — the very gaps the file+bridge walk exists to cover). So a path it
            # finds is real (→ True), but its ABSENCE proves nothing: trusting a fn-graph
            # False silently dropped genuinely-reachable off-chain setters. Never short-
            # circuit on its negative.
            try:
                fg = self._fn_graph_reachable(parent, child)
            except Exception:
                fg = None
            if fg is True:
                self._store_reachable(rkey, True)
                return True
            # AUTHORITATIVE decision: forward reachability over the corpus file-call-graph
            # (the same graph the bridge walk trusts), bounded to max_len and memoized PER
            # SOURCE FILE — a property of the corpus, independent of the API-supplied chain,
            # so it stays O(1) per query no matter how the chain is generated. This replaces
            # the per-(parent,child) backward `is_reachable_bounded` walk, whose unbounded
            # caller-cone exploration was the dense-graph churn. File granularity is a sound
            # OVER-approximation: it never drops a reachable setter (it may keep a few whose
            # file is reachable but whose exact function is not).
            self._ensure_graph()
